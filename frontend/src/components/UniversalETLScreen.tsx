...
import {
  ...
  ArrowLeft, AlertCircle, Rocket, X, FileUp, Download, FileText, Link2,
} from 'lucide-react';
import type {
  ModuleConfig,
  BusinessObject,
...
export function UniversalETLScreen({ module, object, onBack }: UniversalETLScreenProps) {
  ...
  const [resolvedMappingsByEntityName, setResolvedMappingsByEntityName] = useState<Record<string, ResolvedAutoMappingField[]>>({});
  ...
  const handleValidate = useCallback(async () => {
    ...
    try {
      ...
      const resolvedMappings: Record<string, ResolvedAutoMappingField[]> = {};
      ...
      parsedWorkbook.entityNames.forEach(entityName => {
        ...
        if (parsedWorkbook.mode === 'data_only') {
          ...
          resolvedMappings[entityName] = autoGeneratedMapping.fields;
      ...
      setResolvedMappingsByEntityName(resolvedMappings);
      ...
      setResolvedMappingsByEntityName({});
      setValidationReportContent([
        'Validation Report',
        `Workbook: ${file.name}`,
        '',
  ...
  const activeResolvedMappings = activePreviewEntity ? (resolvedMappingsByEntityName[activePreviewEntity.name] ?? []) : [];
            ...
            <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-white/10 rounded-xl p-6 shadow-sm">
              ...
              <div className="space-y-1">
                {entities.map((entity, index) => {
                  ...
                  const resolvedMappingCount = resolvedMappingsByEntityName[entity.name]?.length ?? 0;
                        ...
                        <div>
                          ...
                          {resolvedMappingCount > 0 && (
                            <p className="mt-1 text-xs text-emerald-700 dark:text-emerald-400">
                              Auto-mapped fields: {resolvedMappingCount}
                            </p>
                          )}
                        </div>
                      </div>
              ...
              {activePreviewEntity && activePreviewOutput !== null && activePreviewOutput !== undefined && (
                <div className="mt-6 rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-slate-900/60 overflow-hidden">
                  ...
                  {activeResolvedMappings.length > 0 && (
                    <div className="border-b border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-slate-900/30 px-5 py-4">
                      <div className="inline-flex items-center gap-2 text-sm font-semibold text-slate-900 dark:text-white mb-3">
                        <Link2 size={16} className="text-blue-600 dark:text-blue-400" />
                        Resolved Field Mapping
                      </div>
                      <div className="grid gap-2 sm:grid-cols-2">
                        {activeResolvedMappings.map(field => (
                          <div
                            key={field.outputField}
                            className="rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-950 px-3 py-2"
                          >
                            <p className="text-xs font-semibold text-slate-900 dark:text-white">{field.outputField}</p>
                            <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                              {field.status === 'fixed'
                                ? `Fixed value: ${field.fixedValue}`
                                : field.status === 'mapped'
                                  ? `Excel header: ${field.sourceHeader}`
                                  : `Missing header: ${field.sourceHeader}`}
                            </p>
                          </div>
                        ))}
                      </div>
